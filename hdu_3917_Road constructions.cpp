...
void init()
{
    ...
    for(int i = 1; i <= m; i++ )    out[i].clear();
    memset(sum, 0, sizeof(sum));
    memset(head, -1, sizeof(head));
}
int main()
{
    int x, y, fg, i, j, cp, u, v;
    long long z;
    int cas = 0;
    while(scanf("%d%d", &n, &m)!=EOF)
    {
        if( !(n||m) )    break;
        src = m+1;    init();      des = m+2;
        for( i = 1; i <= m; i++ ){
            scanf("%I64d",&z);
            add(i,des,z);
            s += z;
        ...
        scanf("%d", &n);
        while(n--){
            scanf("%d%d%d%I64d",&u,&v,&cp,&z);
            in[u].insert(cp);
            out[cp].insert(v);
            sum[cp]+=z;
        }

        for( i = 1; i <= m; i++ )    add(src, i, sum[i]);

        for( i = 1; i <= m; i++ ){
            for( it1 = out[i].begin(); it1 != out[i].end(); it1++){
                x = (*it1);
                for( it2 = in[x].begin();    it2 != in[x].end(); it2++){
                    j = (*it2);
                    if(i == j)    continue;
                //    printf(" i = %d\t j = %d\n",i,j);
                    add(j, i, INF);
        ...
        n=m+2;
        rev_BFS();
        long long  ans = s - maxflow() ;
        if( ans < 0 )    ans = 0;
       // printf("s = %I64d\tans = %I64d\n",s,ans);
        printf("%I64d\n", ans);
    }
    return 0;
}
